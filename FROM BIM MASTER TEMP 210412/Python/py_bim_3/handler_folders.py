...
def main_traverse():
    ...
    print(f"---Bạn đang Traverse các folders trong ROOT: {projects_root}")
...
def read_data_folder(folder_data_dir):
    ...
    # newest_data_file = data_files[-1]
    # for f in data_files:
        # f_time = f.split(".json")[0].split("-")[-1]
        # print (datetime.datetime.strptime(f.split(".json")[0].split("-")[-1], time_formatter))        
    # print (pd.read_json(newest_data_file))
